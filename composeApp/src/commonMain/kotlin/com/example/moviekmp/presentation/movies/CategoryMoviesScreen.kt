...
@Composable
fun CategoryMoviesScreen(navController: NavHostController, viewModel: CategoryMoviesViewModel){

    val isDark = isSystemInDarkTheme()
    val moviesUiState by viewModel.allMovies.collectAsStateWithLifecycle()
    val lazyGridState = rememberLazyGridState()

    EndlessLazyGridScrollListener(
        lazyGridState = lazyGridState,
        isLoading = moviesUiState.isLoading,
        hasMoreData = moviesUiState.hasMoreData,
        loadMoreItems = {
            viewModel.getMovies()
        },
    ...
    Column(
        ...
        Column(
            ...
            Row(
                ...
                Text(
                    text = moviesUiState.categoryName ?: "",
                    style = TextStyle(
        ...
        LazyVerticalGrid(
            ...
            items(items = moviesUiState.movies){ item ->
                PosterCellUi(
            ...
            if(moviesUiState.isLoading && moviesUiState.movies.isEmpty()){
                items(count = 12) {
                    PosterCellUi(
                        imagePath = "",
                        title = "",
